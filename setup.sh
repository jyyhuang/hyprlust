...
if ask_confirmation "Do you want to sync config files and wallpapers?"; then
    ...
    if [[ -f "$SCRIPT_DIR/reflector.conf" ]]; then
        echo "Installing reflector.conf..."
    ...
    if [[ -n "$PROFILE" ]]; then
        FIREFOX_PROFILE="$HOME/.config/mozilla/firefox/$PROFILE"
        mkdir -p "$FIREFOX_PROFILE/chrome"
        [[ -f "$SCRIPT_DIR/firefox/user.js" ]] && cp "$SCRIPT_DIR/firefox/user.js" "$FIREFOX_PROFILE/"
        [[ -d "$SCRIPT_DIR/firefox/chrome" ]] && rsync -av "$SCRIPT_DIR/firefox/chrome/" "$FIREFOX_PROFILE/chrome/"
    else
        echo "Could not find Firefox profile, skipping Firefox config sync."
    fi
